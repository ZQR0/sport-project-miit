...
SELECT last_name, first_name, patronymic, birthday, health_groups.name, groups.name from students
	...
	JOIN health_groups ON students.health_group_id = health_groups.id
...
SELECT last_name, first_name, patronymic,
		groups.name as group, health_groups.name as health_group from students
	JOIN visits ON students.id = visits.student_id
	JOIN lessons ON visits.lesson_id = lessons.id
	JOIN groups ON students.group_id = groups.id
...
SELECT DISTINCT t.last_name, t.first_name, t.patronymic,
	...
	JOIN lessons ON t.id = lessons.teacher_id
	JOIN disciplines ON lessons.discipline_id = disciplines.id
	JOIN visits ON lessons.id = visits.lesson_id
	JOIN students ON visits.student_id = students.id
	JOIN groups ON students.group_id = groups.id
